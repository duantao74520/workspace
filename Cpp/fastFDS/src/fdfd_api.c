...
//conf_file配置文件名，myfile上传的文件名，file_id传出参数，文件id
int fdfs_upload_file(const char* conf_file, const char* myfile, char* file_id){
	...
	*group_name = '\0';
	...
	if (result == 0){
        LOG("22", "uploade", "fileID: %s", file_id);
	}
	else{
	...
	tracker_disconnect_server_ex(pTrackerServer, true); //断开连接
	fdfs_client_destroy();  //销毁进程
